...
window.addEventListener('DOMContentLoaded', () => {
  ...
  new MenuCard('img/tabs/vegy.jpg', 'vega', "Меню 'Фитнес'", "Меню 'Фитнес' - это новый подход к приготовлению блюд: больше свежих овощей и фруктов. Продукт активных и здоровых людей. Это абсолютно новый продукт с оптимальной ценой и высоким качеством!", 9, '.menu .container').render();
  ...
  function postData(form) {
    form.addEventListener('submit', e => {
      ...
      fetch('server.php', {
        ...
        console.log(data);
        showThanksModal(message.success); //form.reset();

        statusMessage.remove();
      }).catch(() => {
        showThanksModal(message.failure); //form.reset();
      }).finally(() => {
        form.reset();
      });
    });
